...
describe("Paywall", () => {
  ...
  // The hero leads with a real, user-facing stat rather than burying it in
  // a claim list — worth asserting on directly since it is the page's own
  // headline number.
  it("leads with the ring-delay stat", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText("15")).toBeTruthy();
    expect(getByText(t("heroStatCaption"))).toBeTruthy();
  });

  it("shows every benefit tile at once — nothing here expands or collapses", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
